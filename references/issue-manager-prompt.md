...
Before dispatching the implementation worker or any reviewer, read `<SKILL_PATH>/config/models.json` to decide each role's model. A string value fixes that role to the named model; a `null` or missing value means the role uses the agent's own current model. Do not assign a different model to an unconfigured role.
