...
manager role/model/reasoning matches its manager route, and initialize the state
with that snapshot and actual route. A config file cannot change a manager that is
already running; stop on a mismatch.

First read the complete Issue, repository instructions, linked dependencies, relevant code, and tests. Confirm the clean baseline and inspect the supplied duplicate-work search evidence. Read the supplied ready runtime snapshot before creating any child; verify that your actual manager role/model/reasoning matches its frozen manager route. If it does not, stop because a config file cannot change a manager that is already running. Initialize the state file with that snapshot and the actual manager route. For a bug Issue, reproduce the reported behavior on that baseline before accepting the premise, and use targeted `git blame`, `git log -L`, or `git log -S` evidence to determine whether the behavior is intentional or stale. If it is not reproducible or conflicts with current documented intent, stop before implementation and return the evidence to the root controller. Otherwise translate the Issue into numbered acceptance criteria (`AC-1`, `AC-2`, ...), explicit non-goals, and required validation. Classify the Issue as code-change or verification-only before implementing. If it is verification-only, follow the skill's references/acceptance-already-implemented-pattern.md to collect evidence that the existing implementation already satisfies each acceptance criterion, then stop and report that evidence to the root controller instead of implementing. Use only the supplied Review-policy override as a user-authorized expansion; when it is `none`, apply the skill's default review policy unchanged.

Use exactly one persistent implementation worker for the Issue and one fresh read-only reviewer for each local-review round. Create the separate read-only Judge only for a trigger declared by the frozen strict policy. Release a completed reviewer when supported; otherwise verify that it has no active turn and rely on closure of this top-level task as the final resource-reclamation boundary. Keep the implementation worker available for the entire Issue.

Before dispatching the implementation worker or any reviewer, use only the frozen effective runtime config in the Issue state. Do not reread mutable presets or global runtime config midway through the Issue. Record actual profile/role, provider, model, and reasoning evidence on dispatch. For Hermes, the selected profile remains authoritative and the same durable worker card/profile/provider/model plus complete task context must return for each fresh run.

When the frozen policy requires it, first run `record-assessment` with affected modules, cross-module coupling, algorithm/state/migration difficulty, unresolved decisions, the chosen tier, and the validation plan. A tier suggestion is not assessment evidence. Resolve declared architecture/security uncertainty through the configured separate Judge before worker selection. Create one fresh implementation worker for this Issue. Its role/model/reasoning must exactly match the assessed tier and frozen route. Record the worker route first, then run `start-implementation` immediately before dispatching it so the lifecycle order is audited before code changes begin. The state machine audits declared lifecycle order; it cannot observe arbitrary out-of-band file edits. The worker prompt must require explicit assumptions, the smallest correct change, exact scope and file ownership, TDD for behavior changes, focused and required regression tests, self-review, and a structured development report. Every implementation and later fix must map directly to a numbered acceptance criterion and must not introduce a security framework or speculative abstraction. For a bug fix, require it to inspect same-shaped sibling call sites, repair every affected in-scope sibling, and justify each exclusion without expanding beyond the mapped criterion. Also require the regression test to fail for the expected reason on pre-fix behavior or with the fix temporarily removed, then pass after the fix. Feature-only work is exempt from pre-fix failure proof. Tell the worker that other agents may share the repository and that it must work only in the assigned worktree. It may edit and test but must not commit, push, mutate PRs, merge, delete branches, or remove worktrees. Reuse the same implementation worker for the entire Issue: initial implementation, every local-review fix, remote-feedback classification and fix, and every Issue-caused required-check repair. On Hermes re-dispatch, require the same durable worker handle, same worker profile, same resolved provider/model/reasoning, and complete durable task context before the fresh run acts.

After reading the worker report and inspecting the actual diff, run the script's start-review command with the fresh reviewer's actual role/provider/model/reasoning and create that read-only reviewer. Its id must differ from the worker, all earlier reviewers, and every Judge. Give that reviewer the Issue, numbered acceptance criteria, non-goals, user override if any, instructions, current complete diff, worker report, and test evidence, but no prior reviewer or Judge report or conclusion. Require it to inspect the current code, complete diff, and tests independently before using the worker report to check claimed coverage. Require the skill's default review policy: only realistically reproducible functionality, workflow, data-contract, and error-handling problems that map directly to an acceptance criterion are findings. Extreme constructions, low-probability attack surfaces, general security hardening, and speculative abstractions are not findings unless the user's own instruction explicitly broadens the review. If the reviewer fails before returning a report, record abort-review so the failed attempt does not consume a round. Read every completed report, validate every finding against the code and policy, reject invalid items with reasons, and either close the loop on a clean result or send only accepted findings back to the worker with a precise correction prompt. You may ask that reviewer for evidence or clarification before finishing the same round. Record every review and fix with required report and validation evidence, then release that one-shot reviewer after its `finish-review` or `abort-review` outcome is recorded. Never reuse it in another round. Aborted attempts and check failures do not increment the completed-changes count. Under the strict policy, the second and every later completed `changes` outcome opens a pending adjudication for that round before another fix. Declared reviewer disagreement or architecture/security uncertainty also opens a keyed pending adjudication. Use a separate read-only configured Judge; a resolved decision clears only that decision and does not count as reviewer PASS. A blocked decision stops all affected transitions. Run no more than fifteen completed reviewer rounds. The strict policy always requires a configured-reviewer PASS before PR preparation; if the final round still finds issues, stop blocked without an unreviewed fix or a sixteenth review. Compatibility states without that selected strict policy retain their legacy final-fix behavior.

Run final validation, push, and create a Draft PR whose body contains `Closes #<ISSUE_NUMBER>`. Complete its evidence and mark it Ready for review through the state script. Wait one full 10-minute remote-feedback window. Fetch checks, reviews, review threads, Issue comments, and Copilot comments once, then record that snapshot; the script must accept it before you continue. Keep required checks separate as merge-gate evidence. For every failed required check, attribute it before repair by comparing its logs and scope with the Issue diff and, when uncertain, reproducing it on a clean checkout of the supplied baseline. Re-run once without a code change only with evidence of a genuine flake or transient infrastructure failure; send only Issue-caused, safely repairable failures to the same worker, record the repair with `record-check-repair` using the same recorded worker identity/profile/provider/model, and escalate a failure that still blocks the merge. Give that same worker the numbered acceptance criteria, non-goals, supplied review-policy override, default review policy used by local reviewers, PR HEAD, and fetched feedback. It classifies every fetched review, thread, Issue comment, and Copilot comment and makes only confirmed-safe valid changes. You recheck every disposition against the shared policy, inspect, validate, commit, and push any accepted fix, but do not restart the wait, fetch comments again, or run another review. If that push has a confirmed Issue-caused, safely repairable required-check failure, give the failure evidence to the same worker and record the manager-validated repair HEAD without opening another feedback window. After each fetched inline review thread has a final disposition and accepted fixes are pushed, add a concise disposition reply when needed and resolve the thread through GitHub. Resolve invalid and outdated threads too, but only after documenting why no change is required; do not remove Issue comments or top-level review bodies. Re-query the PR immediately before merge and require zero unresolved review threads. Keep the worker available until merge and cleanup are complete. Merge as soon as required checks, the zero-unresolved-thread gate, and branch protection allow.
...
- the recorded worker role/profile/provider/model/reasoning and a requirement to preserve that exact route across every worker-driven follow-up;
...
```text
python <RUNTIME_SCRIPT> preflight --selection <EXTERNAL_SELECTION> --capabilities <HOST_CAPABILITIES> --output <RUNTIME_SNAPSHOT>
python <SCRIPT> init --state-file <STATE> --issue <N> --branch <BRANCH> --manager-id <ISSUE_TASK_OR_MANAGER_ID> --runtime-snapshot <RUNTIME_SNAPSHOT> --manager-profile <MANAGER_ROLE_OR_PROFILE> --manager-model <MANAGER_MODEL> --manager-reasoning <MANAGER_REASONING>
python <SCRIPT> record-assessment --state-file <STATE> --affected-modules <MODULES> --coupling-and-difficulty <EVIDENCE> --unresolved-decisions <DECISIONS_OR_NONE> --tier simple|normal|complex|extreme --validation-plan <PLAN>
python <SCRIPT> record-worker --state-file <STATE> --worker-id <WORKER_ID> --worker-profile <WORKER_PROFILE> --worker-provider <WORKER_PROVIDER> --worker-model <WORKER_MODEL> --worker-reasoning <WORKER_REASONING> --tier <ASSESSED_TIER> --continuity worker_identity_profile
python <SCRIPT> start-implementation --state-file <STATE> --worker-id <WORKER_ID> --worker-profile <WORKER_PROFILE> --worker-provider <WORKER_PROVIDER> --worker-model <WORKER_MODEL> --worker-reasoning <WORKER_REASONING>
python <SCRIPT> start-review --state-file <STATE> --reviewer-id <REVIEWER_ID> --reviewer-profile <REVIEWER_PROFILE> --reviewer-provider <REVIEWER_PROVIDER> --reviewer-model <REVIEWER_MODEL> --reviewer-reasoning <REVIEWER_REASONING>
python <SCRIPT> abort-review --state-file <STATE> --reason <STARTUP_OR_REPORT_FAILURE>
python <SCRIPT> finish-review --state-file <STATE> --outcome pass|changes --report <REPORT> --summary <SUMMARY> [--adjudication-evidence-key <KEY_FOR_SECOND_OR_LATER_CHANGES>]
python <SCRIPT> request-judge --state-file <STATE> --trigger reviewer_disagreement|architecture_security_uncertainty --evidence-key <KEY> --evidence <EVIDENCE> --revision <REVISION> --report <REPORT>
python <SCRIPT> record-judge --state-file <STATE> --judge-id <JUDGE_ID> --judge-profile <JUDGE_PROFILE> --judge-provider <JUDGE_PROVIDER> --judge-model <JUDGE_MODEL> --judge-reasoning <JUDGE_REASONING> --evidence-key <KEY> --report <REPORT> --outcome resolved|blocked
python <SCRIPT> record-fix --state-file <STATE> --worker-id <WORKER_ID> --worker-profile <WORKER_PROFILE> --worker-provider <WORKER_PROVIDER> --worker-model <WORKER_MODEL> --worker-reasoning <WORKER_REASONING> --report <REPORT> --validation <TEST_EVIDENCE>
...
python <SCRIPT> record-remote-assessment --state-file <STATE> --worker-id <WORKER_ID> --worker-profile <WORKER_PROFILE> --worker-provider <WORKER_PROVIDER> --worker-model <WORKER_MODEL> --worker-reasoning <WORKER_REASONING> --outcome clean|changes|blocked --report <REPORT>
python <SCRIPT> resolve-blocked --state-file <STATE> --decision merge|changes --evidence <USER_OR_CONTROLLER_DECISION>
python <SCRIPT> record-remote-fix --state-file <STATE> --worker-id <WORKER_ID> --worker-profile <WORKER_PROFILE> --worker-provider <WORKER_PROVIDER> --worker-model <WORKER_MODEL> --worker-reasoning <WORKER_REASONING> --head-sha <NEW_SHA> --validation <TEST_EVIDENCE>
python <SCRIPT> record-check-repair --state-file <STATE> --worker-id <WORKER_ID> --worker-profile <WORKER_PROFILE> --worker-provider <WORKER_PROVIDER> --worker-model <WORKER_MODEL> --worker-reasoning <WORKER_REASONING> --head-sha <NEW_SHA> --validation <TEST_EVIDENCE> --evidence <CHECK_REPAIR_EVIDENCE>
python <SCRIPT> record-checks --state-file <STATE> --head-sha <CURRENT_SHA> --result pass|fail --evidence <CHECK_EVIDENCE>
python <SCRIPT> record-review-threads --state-file <STATE> --head-sha <CURRENT_SHA> --unresolved-count <COUNT> --evidence <THREAD_QUERY_EVIDENCE>
...
Treat `<RUNTIME_SNAPSHOT>` as the resolved dispatch contract. Its effective config
contains concrete routes even when the saved current/custom selection uses null
inheritance. Resolve the manager route from this snapshot before creating any
Issue child, and use the same frozen routes for every later worker, reviewer, and
Judge check.
...
- for legacy compatibility states only, any final tested-but-unreviewed fix decision; selected strict policies forbid that transition;
