...
implementation on the default branch (`origin/main`). For those Issues the gap is
...
Decide **before creating the worktree**, on the latest clean `origin/main`:

1. Fetch and read the latest `origin/main`. Do not reason from a stale local branch,
...
reproducible evidence on `origin/main` before the Issue qualifies as
verification-only.
...
- **Commit log on `main`** — the relevant commits (with SHAs and messages) on
  `origin/main` that introduced or carry the satisfying implementation, so the
  evidence traces to a real, merged change rather than an uncommitted assumption.
...
- **`release-record` entries** (where the repository keeps a release record) — the
  ...
  cover.
- **Baseline provenance** — the exact `origin/main` SHA (and fetch time) the audit
  was performed against, so the report is reproducible against a known revision.
...
`origin/main`, the Issue is not verification-only — stop and treat it as a
code-change Issue.
